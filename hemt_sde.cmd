...
;; tChannel: JEDS Fig.1(a) labels this variable. Shinohara's real device uses a
;; 20nm channel ON an AlGaN back-barrier; the JEDS stack instead places a C-doped
;; buffer below. With a C-buffer, a 20nm channel would let C-acceptors deplete the
;; 2DEG (Joshi Part I Fig.13d: ns/Ron recover only beyond ~250nm of separation).
;; Using the JEDS Sec.IV / Part I baseline tChannel = 150 nm keeps the 2DEG intact
;; while matching the figure's stack. Reduce toward 20nm only if pairing with an
;; AlGaN back-barrier instead of the C-buffer.
(define t_chan  0.150)
(define t_cbuf  0.150)
(define t_buf   0.250)
(define t_nuc   0.100)
...
;; n+ regrown ohmic pockets under source and drain. Shinohara [15] used Si-doped
;; n+-GaN regrown ohmics at 7e19 cm^-3 (50 nm), giving access R ~ 0.1 ohm.mm and
;; Ron = 0.29 ohm.mm. Use 7e19 so the S/D contacts are low-resistance and tie to
;; the 2DEG. Windows span from cap top to just below the barrier.
